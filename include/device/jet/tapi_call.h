...
#ifndef __DEVICE_JET_CALL_H__
#define __DEVICE_JET_CALL_H__

/**
 * All the TAPI call context structures specific to Jet will be defined here.
 *
 * The values below are copied from the Wave protocol definitions; the Jet
 * modem uses the same Mocha AP-CP TAPI call protocol so the opcodes are
 * identical.  They are placed here rather than in the shared tapi_call.h so
 * that device-specific overrides remain possible in the future.
 */
enum TAPI_CALL_TYPE
{
